...
async function getHandler(req: NextRequestWithUser) {
  ...
  const result = await listMyConnections(userId, validated.data, tz);
...
async function postHandler(req: NextRequestWithUser) {
  ...
  const validated = validateData(brokerConnectionSaveSchema, body);
  if (!validated.status) return sendResult(validated);

  const result = await createMyConnection(userId, validated.data);
  return sendResult(result);
}
