import { resendNewEmailOtp } from "@/server/modules/account/email-change.service";
import { validateData } from "@/server/lib/validator";
import { rateLimit } from "@/server/lib/rateLimit";
import { getClientIp } from "@/server/utils/clientInfo";
import { emailChangeResendSchema } from "@/modules/account/email.validator";
...
async function handler(req: NextRequestWithUser) {
  ...
  const validated = validateData(emailChangeResendSchema, body);
  if (!validated.status) return sendResult(validated);

  const userId = req.user?.id;
  if (!userId) return sendError(401, "Unauthorized");

  return sendResult(await resendNewEmailOtp(userId, validated.data.handle));
}
