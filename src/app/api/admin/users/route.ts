...
import { listUsers } from "@/server/modules/admin/user/user.service";
...
import { userCreateSchema } from "@/modules/account/user.validator";
...
async function getHandler(req: NextRequestWithAdmin) {
  ...
  const res = await listUsers(validated.data, getClientTimezone(req));
...
async function postHandler(req: NextRequestWithAdmin) {
  ...
  const validated = validateData(userCreateSchema, fields);
  if (!validated.status) return sendResult(validated);
