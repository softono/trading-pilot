...
export function SidebarNav({ items, activeTab }: SidebarNavProps) {
  // Determine active tab by prop or fallback to pathname
  let activeValue = "";
  if (activeTab) {
  ...
  const normalizedActiveValue = normalizePath(activeValue);
  ...
  return (
    <div className="pt-3">
      ...
      <ul className="hidden md:flex flex-row gap-2 w-full mb-6">
        {items.map((item) => {
          const itemHrefNormalized = normalizePath(item.href);
          const isActive =
            itemHrefNormalized === normalizedActiveValue ||
            normalizedActiveValue.startsWith(`${itemHrefNormalized}/`);
          return (
            <li key={item.href}>
              <Link
                href={item.href}
